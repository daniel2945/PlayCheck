...
export default function GameDetails() {
  ...
  if (error || !game) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen pt-24 px-4 text-center bg-[#202124]">
        ...
        <button onClick={() => navigate("/catalog")} className="px-8 py-3 bg-[#8ab4f8] text-[#202124] rounded-full font-bold hover:bg-[#aecbfa]">
          Back to Catalog
        </button>
      </div>
    );
  ...
  const heroImage = game.background_image || game.image || "https://placehold.co/1920x1080/1a1a1a/ffffff?text=No+Image+Available";
  
  // עיבוד התאריך המדויק (לפי releasedDate שמגיע מהשרת/מונגו)
  const rawDate = game.releasedDate;
  const releasedDate = rawDate && rawDate !== "TBA"
...
>>>>>>> fcde8e3109dccda3b8ec10880406b049b8b00542
        ...
        <div className="flex-1">
          <h2 className="text-2xl text-[#8ab4f8] font-bold mb-4 border-b border-[#5f6368] pb-2">About the Game</h2>
          ...
          <div className="text-[#e8eaed] text-lg leading-relaxed space-y-4">
            ...
            {game.description_raw ? (
              ...
              <div dangerouslySetInnerHTML={{ __html: game.description || "No description available for this game." }} />
            )}
          </div>
        </div>
        ...
        <div className="w-full md:w-80 flex flex-col gap-6">
          <div className="bg-[#303134] p-6 rounded-2xl border border-[#5f6368] shadow-xl sticky top-24">
            <h3 className="text-[#e8eaed] text-xl font-medium mb-6 text-center">Ready to play?</h3>
